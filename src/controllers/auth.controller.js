...
class AuthController {
    ...
    async resetPasswordRequest(request, response) {
        try {
            ...
            const reset_link = `http://localhost:5173/reset-password?token=${token}`;
            ...
            return response.status(200).json({
                ok: true,
                message: "Si el correo existe, recibiras un mail para restablecer tu contraseña"
            });
        } catch (error) {
            if (error instanceof ServerError) {
    ...
    async resetPassword(request, response) {
        try {
            ...
            if (!token || !password) {
                throw new ServerError("El token y la nueva contraseña son obligatorios", 400);
            }

            const decodePayload = jwt.decode(token);

            if (!decodePayload || !decodePayload.email) {
                throw new ServerError("El usuario no existe", 400);
            }

            const user = await userRepository.getByEmail(decodePayload.email);
            if (!user) {
                throw new ServerError("El usuario no existe", 400);
            }
            ...
            try {
                ...
                throw new ServerError("El token ha expirado o ya fue usado", 401);
            }
            ...
            if (error instanceof ServerError) {
                ...
                console.error("Error critico en reset password: ", error);
                return response.status(500).json({
                    message: "Error interno del servidor",
                    ok: false,
                    status: 500
